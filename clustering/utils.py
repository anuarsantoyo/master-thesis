...
def get_cluster_input_data(scaler = MinMaxScaler(), data='fa_data', start='2020-08-01', end='2021-02-01', start_train='2020-05-28', end_train='2021-12-02'):
  if data == 'pca_data':
      data_path = 'data/preprocessing/dim_reduction/pca_data.csv'
      cluster_input_cols = []
      ...
      data_path = 'data/preprocessing/dim_reduction/fa_data.csv'
  ...
  info_dict = {'data_path': data_path, 'cluster_input_cols': cluster_input_cols, 'scaler_type':  scaler.__str__(), 'data': data,  'start': start, 'end': end, 'start_train': start_train, 'end_train': end_train}

  return df, cluster_input_train, cluster_input_all, info_dict

def get_cluster_data(n_cluster=2, method='gmm', cov_type='full', cluster_input_data='fa_data', start='2020-08-01', end='2021-02-01', start_train='2020-08-01', end_train='2020-12-01', random_seed=1, n_init=10):

  df, answers_train, answers, info_dict = get_cluster_input_data(data=cluster_input_data, start=start, end=end, start_train=start_train, end_train=end_train)
...
def get_cluster_input_data_experiments(scaler = MinMaxScaler(), data='fa_data', start_train='2020-08-01', end_train='2020-12-01'): # Anuar here NNMF
  if data == 'pca_data':
      ...
      data_path = 'data/preprocessing/dim_reduction/fa_data_experiments.csv' #use extra short file to save time
      cluster_input_cols = factor_cols
...
def get_cluster_data_experiments(n_cluster=2, method='gmm', cov_type='full', cluster_input_data='fa_data', start_train='2020-08-01', end_train='2020-12-01', random_seed=1, n_init=10):
  ...
  if method == 'kmeans':
      ...
      for i in range(n_init):
          model = NMF(n_components=n_cluster, init='random', random_state=random_seed, max_iter=1000000)
          model.fit(cluster_input_train)
      ...
      kmeans = KMeans(n_clusters=n_cluster, random_state=random_seed).fit(best_model.transform(cluster_input_all))
